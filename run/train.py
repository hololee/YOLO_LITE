...
for epoch in range(cfg.TOTAL_EPOCH):
    ...
    ax[0].plot([i for i in range(len(train_loss))], train_loss, linestyle='-', dash_joinstyle='round')
    ax[0].grid(True, alpha=0.3)

    ax[1].set_title('Develop set loss', fontsize=13)
    ax[1].plot([i for i in range(len(dev_loss))], dev_loss, color='red', linestyle='-', dash_joinstyle='round')
